...
def get_args():
    ...
    parser.add_argument("--db", type=str, default='dataset_main',
                        help="path to database")
    parser.add_argument("--output", type=str, default='biwi_dataset_128_full.npz',
        ...
        for j in tqdm(range(len(onlyfiles_png[i]))):
            ...
            if len(detected) > 0:
                ...
                if dis_betw_frames < 80 or j == 0:
                    ...
                    folder.append(i)
                    img_name = mypath_obj + '/' + img_name
                    image_names.append(img_name)

    np.savez(output_path, image=np.array(out_imgs), pose=np.array(out_poses), img_size=img_size,folder_name=np.array(folder), image_name=np.array(image_names))
